...
const City: FC = () => {
    ...
    const { state } = useLocation<WeatherData>();
    ...
    const cityObject: ITableData<string> | null = useMemo(() => {
        if (typeof city === 'string') {
            const weatherData = state ?? cities.find(c => c.city.name.toLowerCase() === city.toLowerCase());
            if (weatherData) {
                return {
                    ...weatherData,
                    city: weatherData.city.name,
                    picture: weatherData.city.picture,
                    temp: temperatureConverter(weatherData.temp, weatherData.tempType, 1)
                };
            }
        }
        return null;
